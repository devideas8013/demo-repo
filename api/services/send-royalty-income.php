...
$service_sql = "SELECT * FROM allservices WHERE service_name='ROYALTY_INCOME' ";
...
if(mysqli_num_rows($service_result) > 0){
    ...
    $service_royalty_val = $service_res_data['service_value'];
    
    if($service_royalty_val > 0){
      ...
      if (mysqli_num_rows($select_result) > 0) {
       ...
       while ($row = mysqli_fetch_assoc($select_result)) {
        ...
        if ($total_refer_record >= 100) {
            
            $reward_bonus = $service_royalty_val;
            $extra_msg = "Royalty Income";
            $currIndex++;
       ...
       if ($currIndex > 0) {
        echo "All Royalty income sent! (success)<br>Total (Transactions): " .
            $currIndex;
       } else {

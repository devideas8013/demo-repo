...
if (mysqli_num_rows($select_result) > 0) {
    ...
    while ($row = mysqli_fetch_assoc($select_result)) {
        ...
        $select_transaction_sql = "SELECT * FROM othertransactions WHERE type='directbonus' AND receive_from='{$user_uniq_id}' ";
        ...
        if (mysqli_num_rows($select_transaction_result) <= 0) {
         ...
         if (mysqli_num_rows($user_record_result) > 0) {
            ...
            $initiateCommission = new InitiateCommission($user_uniq_id,$user_refered_by,"directbonus","Refer Bonus",true);
    ...
    if ($currIndex > 0) {
        echo "All Direct Bonus sent! (success)<br>Total (Transactions): " .
            $currIndex;
    } else {
